...
export async function atualizarTabelasEDadosFinanceiro(ordem, desc){
    ...
    auth.onAuthStateChanged(async (userCredentials) => {
        ...
        let qIniciais;
        if(desc == ""){
            q = await query(collection(db, "Emprestimos"), where("userID", "==", userCredentials.uid), where("parcelas", "array-contains", `${ano}-${mes}`), orderBy(ordem))

            qIniciais = await query(collection(db, "Emprestimos"), where("userID", "==", userCredentials.uid), where("mesInicio", "==", `${ano}-${mes}`), where("faturaDoMes", "==", false), orderBy(ordem))
        } else {
            q = await query(collection(db, "Emprestimos"), where("userID", "==", userCredentials.uid), where("parcelas", "array-contains", `${ano}-${mes}`), orderBy(ordem, desc));

            qIniciais = await query(collection(db, "Emprestimos"), where("userID", "==", userCredentials.uid), where("mesInicio", "==", `${ano}-${mes}`), where("faturaDoMes", "==", false), orderBy(ordem, desc))
        }
